...
public class JwtTokenCustomizer implements OAuth2TokenCustomizer<JwtEncodingContext> {
    ...
    public void customize(JwtEncodingContext context) {
        String tokeType = context.getTokenType().getValue();
        AuthorizationGrantType authorizationGrantType = context.getAuthorizationGrantType();

        if (isIdToken(tokeType)) {
            customizeIdToken(context);
        } else if (isAccessToken(tokeType) && isUserDelegatedFlow(authorizationGrantType)) {
    ...
    private void customizeAccessToken(JwtEncodingContext context) {
        ...
        Set<String> filteredScopes = scopePolicyService
                .resolveScopes(AuthUserType.valueOf(role), clientId, context.getAuthorizedScopes());
    ...
    private boolean isAccessToken(String tokeType) {
        return OAuth2TokenType.ACCESS_TOKEN.getValue().equals(tokeType);
    }

    private boolean isIdToken(String tokeType) {
        return OidcParameterNames.ID_TOKEN.equals(tokeType);
    }
}
